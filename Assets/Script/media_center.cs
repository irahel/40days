...
public class media_center : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0) && mouse_in && event_here && !active && !controller.solving_event && first_opening)
        {
            ...
            canvas_item.GetComponent<canvas_media>().make_question(controller.current_week);
            active = true;
            first_opening = false;
        }
    ...
    public void closed_clicked()
    {
        if (active)
        {
            canvas_item.GetComponent<canvas_media>().closed();
    ...
    public void next_week()
    {
        ...
        canvas_item.SetActive(true);
        canvas_item.GetComponent<canvas_media>().next_week();
        canvas_item.SetActive(false);
    }
